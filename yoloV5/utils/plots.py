...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    #     c2 = c1[0] + t_size[0], c1[1] - t_size[1] + 50
    #     cv2.rectangle(img, c1, c2, color, 2, cv2.LINE_AA)  # filled # cv2.LINE_AA
    #     # cv2.putText(img, label, (c1[0], c1[1] + 27 - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)


def plot_one_box_xy(x, xy, img, color=None, label=None, line_thickness=None):
    # Plots one bounding box on image img
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    tl = 2
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    # cv2.rectangle(img, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)
    x = int(xy[0]*img.shape[1])
    y = int(xy[1]*img.shape[0])
